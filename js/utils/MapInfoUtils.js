...
import JSONViewer from '@mapstore/components/data/identify/viewers/JSONViewer';
...
import wmts from '@mapstore/utils/mapinfo/wmts';
import { INFO_FORMATS, INFO_FORMATS_BY_MIME_TYPE, Validator } from '@mapstore/utils/FeatureInfoUtils';
import vector from '@mapstore/utils/mapinfo/vector';
import threeDTiles from '@mapstore/utils/mapinfo/threeDTiles';

import wfs from '@js/utils/mapinfo/wfs';
import wms from '@js/utils/mapinfo/wms';
...
export const getDefaultInfoFormatValueFromLayer = (layer, props) => {
    ...
    return layerFormat && INFO_FORMATS[layerFormat]
        || props.format
        || MapInfoUtils.getDefaultInfoFormatValue();
};

export const getLayerGfiViewer = (layer, options) => {
    const gfiType = options.gfiType || 'featureInfo';

    if (layer[gfiType] && layer[gfiType].viewer) {
 ...
 * @return {object} feature info options
 */
export const getLayerGfiOptions = (layer) => {
    return {
