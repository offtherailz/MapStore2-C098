...
import { isPluginInContext } from '@mapstore/selectors/context';
import { currentLocaleSelector } from '@mapstore/selectors/locale';
import { isCesium } from '@mapstore/selectors/maptype';
import {isIdentifyAvailable} from "@mapstore/selectors/controls";
 ...
 */
export const generalInfoFormatObjectSelector = (state) => {
    const infoFormat = get(state, "mapInfo.configuration.infoFormat", "text/plain");
    return isString(infoFormat) ? {featureInfo: infoFormat} : infoFormat;
};
export const createInfoFormatSelector = gfiType => createSelector(generalInfoFormatObjectSelector, infoFormat => infoFormat[gfiType] || "text/plain");
export const generalInfoFormatSelector = createInfoFormatSelector('featureInfo');
export const mapTipFormatSelector = createInfoFormatSelector('mapTip');
export const mapTipFormatEnabledSelector = state => state?.mapInfo?.enableMapTipFormat;
export const identifyGfiTypeSelector = state => state?.mapInfo?.gfiType;
export const featureInfoClickFormatSelector = createSelector(generalInfoFormatSelector, mapTipFormatSelector, identifyGfiTypeSelector,
    (infoFormat, mapTipFormat, gfiType) => gfiType === 'mapTip' ? mapTipFormat : infoFormat);
export const mapTipActiveLayerIdSelector = state => state?.mapInfo?.configuration?.mapTipActiveLayerId;
