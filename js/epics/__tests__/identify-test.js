...
const { ZOOM_TO_POINT, clickOnMap, CHANGE_MAP_VIEW, UNREGISTER_EVENT_LISTENER, REGISTER_EVENT_LISTENER} = require('../../../MapStore2/web/client/actions/map');
const { FEATURE_INFO_CLICK, UPDATE_CENTER_TO_MARKER, PURGE_MAPINFO_RESULTS, NEW_MAPINFO_REQUEST, LOAD_FEATURE_INFO, NO_QUERYABLE_LAYERS, ERROR_FEATURE_INFO, EXCEPTIONS_FEATURE_INFO, SHOW_MAPINFO_MARKER, HIDE_MAPINFO_MARKER, GET_VECTOR_INFO, SET_CURRENT_EDIT_FEATURE_QUERY, CLEAR_WARNING, SET_GFI_TYPE, loadFeatureInfo, featureInfoClick, closeIdentify, toggleHighlightFeature, editLayerFeatures, updateFeatureInfoClickPoint, purgeMapInfoResults } = require('../../actions/mapInfo');
const { REMOVE_MAP_POPUP } = require('../../../MapStore2/web/client/actions/mapPopups');
const { TEXT_SEARCH_CANCEL_ITEM } = require('../../../MapStore2/web/client/actions/search');
const {
...
const { CLOSE_ANNOTATIONS } = require('../../../MapStore2/web/client/actions/annotations');
const { testEpic, TEST_TIMEOUT, addTimeoutEpic } = require('../../../MapStore2/web/client/epics/__tests__/epicTestUtils');
const { registerHook, GET_COORDINATES_FROM_PIXEL_HOOK, GET_PIXEL_FROM_COORDINATES_HOOK } = require('../../../MapStore2/web/client/utils/MapUtils');
const { setControlProperties } = require('../../../MapStore2/web/client/actions/controls');
const { BROWSE_DATA } = require('../../../MapStore2/web/client/actions/layers');
...
describe('identify Epics', () => {
        ...
        const state = {
            layers: {
                flat: [{
                    ...
                    "title": "TITLE",
                    type: "wms",
                    url: 'base/MapStore2/web/client/test-resources/featureInfo-response.json'
                }]
            }
        };
    ...
    it('getFeatureInfoOnFeatureInfoClick WMS', (done) => {
        ...
        testEpic(getFeatureInfoOnFeatureInfoClick, NUM_ACTIONS, sentActions, (actions) => {
            try {
                ...
                const [a0, a1, a2, a3, a4, a5, a6] = actions;
                expect(a0).toExist();
                expect(a0.type).toBe(PURGE_MAPINFO_RESULTS);
                expect(a1).toExist();
                expect(a1.type).toBe(CLEAR_WARNING);
                expect(a2).toExist();
                expect(a2.type).toBe(SET_GFI_TYPE);
                expect(a2.gfiType).toBe('featureInfo');
                expect(a3).toExist();
                expect(a3.type).toBe(NEW_MAPINFO_REQUEST);
                expect(a3.reqId).toExist();
                expect(a3.request).toExist();
                expect(a4).toExist();
                expect(a4.type).toBe(NEW_MAPINFO_REQUEST);
                expect(a4.reqId).toExist();
                expect(a4.request).toExist();
                expect(a5).toExist();
                expect(a5.type).toBe(LOAD_FEATURE_INFO);
                expect(a5.data).toExist();
                expect(a5.requestParams).toExist();
                expect(a5.reqId).toExist();
                expect(a5.layerMetadata.title).toBe(state.layers.flat[a5.requestParams.id === "TEST" ? 0 : 1].title);
                expect(a6).toExist();
                expect(a6.layerMetadata.title).toBe(state.layers.flat[a6.requestParams.id === "TEST" ? 0 : 1].title);
                done();
            } catch (ex) {
                done(ex);
            }
    ...
    it('getFeatureInfoOnFeatureInfoClick WMS with filteredList and override params', (done) => {
        ...
        testEpic(getFeatureInfoOnFeatureInfoClick, 5, sentActions, ([a0, a1, a2, a3, a4]) => {
            try {
                ...
                expect(a4.data.features).toExist();
                expect(a4.data.features.length).toBe(1);
                expect(a4.requestParams).toExist();
                expect(a4.reqId).toExist();
                expect(a4.layerMetadata.title).toBe(state.layers.flat[0].title);
                done();
            } catch (ex) {
                done(ex);
            }
    ...
    it('getFeatureInfoOnFeatureInfoClick with multiSelection', (done) => {
        ...
        testEpic(getFeatureInfoOnFeatureInfoClick, 3, sentActions, ([a1, a2, a3]) => {
            try {
                // no purge
                expect(a1).toExist();
                expect(a1.type).toBe(SET_GFI_TYPE);
                expect(a1.gfiType).toBe('featureInfo');
                expect(a2).toExist();
                expect(a2.type).toBe(NEW_MAPINFO_REQUEST);
                expect(a2.reqId).toExist();
                expect(a2.request).toExist();
                expect(a2.request.cql_filter).toNotExist();
                expect(a3).toExist();
    ...
    it('getFeatureInfoOnFeatureInfoClick triggers ERROR_FEATURE_INFO on load error', (done) => {
        ...
        testEpic(getFeatureInfoOnFeatureInfoClick, 5, sentActions, ([a0, a1, a2, a3, a4]) => {
            try {
                ...
                expect(a3.request).toExist();
                expect(a4).toExist();
                expect(a4.type).toBe(ERROR_FEATURE_INFO);
                expect(a4).toExist();
                expect(a4.type).toBe(ERROR_FEATURE_INFO);
                expect(a4.error).toExist();
                expect(a4.reqId).toExist();
                expect(a4.requestParams).toExist();
                expect(a4.layerMetadata.title).toBe(state.layers.flat[0].title);
                done();
    ...
    it('getFeatureInfoOnFeatureInfoClick handle server exception', (done) => {
        ...
        const state = {
            ...
            layers: {
                flat: [{
                    ...
                    type: "wms",
                    visibility: true,
                    url: 'base/MapStore2/web/client/test-resources/featureInfo-exception.json'
                }]
            }
        };
        const sentActions = [featureInfoClick({ latlng: { lat: 36.95, lng: -79.84 } })];
        testEpic(getFeatureInfoOnFeatureInfoClick, 5, sentActions, ([a0, a1, a2, a3, a4]) => {
            try {
                ...
                expect(a3.request).toExist();
                expect(a4).toExist();
                expect(a4.type).toBe(EXCEPTIONS_FEATURE_INFO);
                expect(a4.exceptions).toExist();
                expect(a4.reqId).toExist();
                expect(a4.requestParams).toExist();
                expect(a4.layerMetadata.title).toBe(state.layers.flat[0].title);
                done();
    ...
    it('Test local request, remote request and skip background layers', done => {
        ...
        const sentActions = [featureInfoClick({ latlng: { lat: 36.95, lng: -79.84 } })];
        testEpic(getFeatureInfoOnFeatureInfoClick, 6, sentActions, ([a0, a1, a2, a3, a4, a5]) => {
            try {
                expect(a0).toExist();
                expect(a0.type).toBe(PURGE_MAPINFO_RESULTS);
                expect(a1).toExist();
                ...
                expect(a2.gfiType).toBe('featureInfo');
                expect(a3).toExist();
                expect(a3.type).toBe(GET_VECTOR_INFO);
                expect(a4.type).toBe(NEW_MAPINFO_REQUEST);
                expect(a4.reqId).toExist();
                expect(a4.request).toExist();
                expect(a5).toExist();
